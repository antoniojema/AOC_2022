import { getMap, Path } from './elevation.js'
...
async function main() {
    ...
    const dist = map.findShortestDistance(
        final_pos,
        (from, to, elev) => elev[from.row][from.col] <= elev[to.row][to.col]+1,
        (pos, elev) => elev[pos.row][pos.col] == 0
    );
